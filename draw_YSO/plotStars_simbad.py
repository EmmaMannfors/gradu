...
for filename in os.listdir(list_in):
	...
	#		continue
		...
		f.show_colorscale(cmap='viridis',vmin=0) #'jet_r', vmin=0,vmax=(number)
		...
		with open(list_in+filename) as coList:
			...
			for line in coList:
				...
				starType = p[4]
				if starType != '*' and starType != 'Or*' and starType != 'TT*' and starType != 'FU*'and starType != 'TT?':
					continue
				...
				#print ra
				#print dec
				if starType == 'Or*': # Plot variable Orionis
					try:
						f.show_circles(ra, dec, 0.005,color='white')
						f.add_label(ra,dec,'Or',color='white',size=10)	
					except:
					...
					try:
						...
						print '\n'+'Unable to draw plot\n'+starType+'	'+str(ra)+'	'+str(dec)+'\n'+10*'*'
				elif starType == 'FU*': # Plot FU Ori type star
					try:
						f.show_circles(ra, dec, 0.005,color='white')
						f.add_label(ra,dec,'FU',color='white',size=10)	
					except:
						print '\n'+'Unable to draw plot\n'+starType+'	'+str(ra)+'	'+str(dec)+'\n'+10*'*'
				elif starType == 'TT?': # Plot TT candidates
					try:
						f.show_circles(ra, dec, 0.005,color='white')
						f.add_label(ra,dec,'TT?',color='white',size=8)	
					except:
						print '\n'+'Unable to draw plot\n'+starType+'	'+str(ra)+'	'+str(dec)+'\n'+10*'*'
