...
in_160 = '/home/emma/gradu/data/All_data/PACS_160_Herschel/reproject_160_to_500/'
in_250 = '/home/emma/gradu/data/All_data/SPIRE_PSW_250_Herschel/reproject_250/'
in_350 = '/home/emma/gradu/data/All_data/SPIRE_PMW_350_Herschel/reproject_350/'
...
def change(filename):
	if filename == 'NA':
		return 'NA'
	if filename.endswith('_crop.fits'):
		n = filename.strip('_crop.fits')+'_resize_1.fits'
		return n
	elif filename.endswith('_crop_2.fits'):
		n = filename.strip('_crop_2.fits')+'_resize_2.fits'
		return n
	elif filename.endswith('_crop_3.fits'):
		n = filename.strip('_crop_3.fits')+'_resize_3.fits'
		return n
	elif filename.endswith('_crop_4.fits'):
		n = filename.strip('_crop_4.fits')+'_resize_4.fits'
		return n
...
def outName(filename):
	...
	elif filename.endswith('resize_1.fits'):
		outName = filename.strip('_resize_1.fits')+('_norm_1.fits')
	elif filename.endswith('res_2.fits'): 
		outName = filename.strip('_res_2.fits')+('_norm_2.fits')
	elif filename.endswith('resize_2.fits'):
		outName = filename.strip('_resize_2.fits')+('_norm_2.fits')
	elif filename.endswith('res_3.fits'): 
		outName = filename.strip('_res_3.fits')+('_norm_3.fits')
	elif filename.endswith('resize_3.fits'):
		outName = filename.strip('_resize_3.fits')+('_norm_3.fits')
...
#Dropbox: Readme
normMatches = open('normalized_matches_500_160.txt','w')
normMatches.write('PACS 160		SPIRE 250		SPIRE 350		SPIRE500\n')
########################################
...
#			continue
	####
		...
		pmw = in_350+change(matchList[4])
		plw = in_500+matchList[5]
		SCUBA = in_850+scuba
		locList = [pacs160,psw,pmw,plw]
